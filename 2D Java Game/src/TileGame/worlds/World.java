...
public class World {
	...
	public World(Handler handler, String world, String data, String inventory){
		...
		entityManager.addEntity(new STShop(handler, 18 * Tile.TILEWIDTH, 20 * Tile.TILEHEIGHT, width, height, 0, 0, 22));

		//trees
		entityManager.addEntity(new Fern(handler, 15 * Tile.TILEWIDTH, 6 * Tile.TILEHEIGHT, 3, true));
		entityManager.addEntity(new Tree(handler, 2 * Tile.TILEWIDTH, 8 * Tile.TILEHEIGHT, 4, true));
		entityManager.addEntity(new Tree(handler, 3 * Tile.TILEWIDTH, 8 * Tile.TILEHEIGHT, 5, true));
		entityManager.addEntity(new Tree(handler, 4 * Tile.TILEWIDTH, 8 * Tile.TILEHEIGHT, 6, true));
		//bricks
		entityManager.addEntity(new BrickWall(handler, 9 * Tile.TILEWIDTH, 5 * Tile.TILEHEIGHT, 7, true));
		entityManager.addEntity(new BrickWall(handler, 9 * Tile.TILEWIDTH, 6 * Tile.TILEHEIGHT, 8, true));
		entityManager.addEntity(new BrickWall(handler, 9 * Tile.TILEWIDTH, 7 * Tile.TILEHEIGHT, 9, true));
		entityManager.addEntity(new BrickWall(handler, 6 * Tile.TILEWIDTH, 3 * Tile.TILEHEIGHT, 10, true));
		
		//chests
		entityManager.addEntity(new Chest(handler, 6 * Tile.TILEWIDTH, 1 * Tile.TILEHEIGHT, 11, true));
		
		//enemies
		entityManager.addEntity(new CatEnemy(handler, 17 * Tile.TILEWIDTH, 4 * Tile.TILEHEIGHT, 12));
		entityManager.addEntity(new CatEnemy2(handler, 13 * Tile.TILEWIDTH, 13 * Tile.TILEHEIGHT, 13));		
		//blockades
		entityManager.addEntity(new Blockcade(handler, 9 * Tile.TILEWIDTH, 11 * Tile.TILEHEIGHT, 17, false));
		entityManager.addEntity(new Blockcade(handler, 9 * Tile.TILEWIDTH, 12 * Tile.TILEHEIGHT, 18, false));
		entityManager.addEntity(new Blockcade(handler, 9 * Tile.TILEWIDTH, 13 * Tile.TILEHEIGHT, 19, false));
		entityManager.addEntity(new Blockcade(handler, 9 * Tile.TILEWIDTH, 14 * Tile.TILEHEIGHT, 20, false));
		
		entityManager.addEntity(new Blockcade(handler, 10 * Tile.TILEWIDTH, 18 * Tile.TILEHEIGHT, 14, false));
		entityManager.addEntity(new Blockcade(handler, 11 * Tile.TILEWIDTH, 18 * Tile.TILEHEIGHT, 15, false));
		entityManager.addEntity(new Blockcade(handler, 12 * Tile.TILEWIDTH, 18 * Tile.TILEHEIGHT, 16, false));
		
		//Bosses
		entityManager.addEntity(new VampireBoss(handler, 4 * Tile.TILEWIDTH, 19 * Tile.TILEHEIGHT, 21));
	...
	public void loadWorld(String path, String data, String inventory){
		...
		if(MenuState.load == true){
			...
			for(int x = 0; x < (id * 3) + 3; x++){
				...
				GameState.level = Utils.parseInt(token[(id * 3) + 4]);
			...
			for(int x = 1; x < (itemId * 2) + 1; x++){
				...
				}else if((Utils.parseInt(tokens3[x]) == 5)){
					handler.getWorld().getEntityManager().getPlayer().getInventory().
					addItem(Item.hPotItem.createNew(Utils.parseInt(tokens3[x + 1])));
				}else if((Utils.parseInt(tokens3[x]) == 6)){
					handler.getWorld().getEntityManager().getPlayer().getInventory().
					addItem(Item.mPotItem.createNew(Utils.parseInt(tokens3[x + 1])));
				}else if((Utils.parseInt(tokens3[x]) == 7)){
					handler.getWorld().getEntityManager().getPlayer().getInventory().
					addItem(Item.bowItem.createNew(Utils.parseInt(tokens3[x + 1])));
				}
				x+= 1;
			}
